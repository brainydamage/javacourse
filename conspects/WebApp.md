...
**[Web и App сервер](#web&appserver)**
...
RUNNABLE - уже выполняется (выполняется метод run()), или готов начать выполняться (run() еще не вызван)  
...
```
ThreadPoolExecutor( 
int corePoolSize, //минимально желаемое число потоков в пуле
int maximumPoolSize, //максимально желаемое число потоков в пуле
long keepAliveTime, //время ожидания перед завершение лишнего простаивающего потока
TimeUnit unit, 
BlockingQueue<Runnable> workQueue)
```

Манипуляция количеством потоков:
 * в этот момент существует меньше потоков, чем corePoolSize -> создается новый поток для обработки запроса, даже если другие рабочие потоки неактивны;  
* в этот момент запущено больше потоков, чем corePoolSize, но меньше, чем maximumPoolSize -> то новый поток будет создан только если очередь заполнена;
* в этот момент запущено больше потоков, чем corePoolSize, и лишние простаивают больше, чем keepAliveTime -> то лишние потоки будут удалены;
* в этот момент запущено больше потоков, чем maximumPoolSize, и очередь заполнена:
    * метод выбросит RejectedExecutionException, 
    * начнет выполнять задачу сам, 
    * скипнет эту задачу, 
    * удалит самую старую задачу (следующую в очереди) или
    * выполнит какую-то кастомную логику.

Внутри ThreadPoolExecutor есть Worker'ы, которые постоянно опрашивают очередь внутри пула (BlockingQueue) и забирают задачи из нее. То есть метод runWorker() сам никогда не завершается и живет и работает, пока его не убьют сверху.